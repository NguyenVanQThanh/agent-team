...
**Context:** If working in an isolated worktree, it was created by the agent-team's tournament flow (`spawn-team.sh` → `.Codex/team/worktrees/`), not by a separate worktree skill.
...
slice it into `.Codex/team/tasks.md` rows and spawn devs in parallel via
`.Codex/bin/spawn-team.sh` (≥ 2 devs per call). Execution is orchestrated by the
...
> agent-team leader is the single orchestration layer (see ARCHITECTURE §4).
