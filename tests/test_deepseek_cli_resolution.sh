...
make_broken_stub "$broken_bin_dir" codewhale
make_stub "$broken_bin_dir" deepseek-tui
...
assert_eq "deepseek-tui" "$(PATH="$broken_bin_dir" DEEPSEEK_BIN='' deepseek_resolve_bin)" "skips a broken codewhale installation"
...
echo "PASS: codewhale preference, broken-install fallback, legacy fallback, override, missing-CLI error, and doctor reporting"
